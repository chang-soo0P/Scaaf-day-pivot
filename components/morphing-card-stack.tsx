...
export interface CardData {
  ...
  icon?: ReactNode
  color?: string
  // Extended fields for topic issues
  sourceName?: string
              ...
              return (
                <motion.div
                  ...
                  className={cn(
                    ...
                    isExpanded && "ring-2 ring-primary rounded-2xl",
                  )}
                >
                  {renderCard ? (
                    renderCard(card, layout, isTopCard, isExpanded)
                  ) : (
                    <DefaultCard card={card} layout={layout} isTopCard={isTopCard} />
                  )}
                </motion.div>
              )
            })}
...
function DefaultCard({ card, layout, isTopCard }: { card: CardData; layout: LayoutMode; isTopCard: boolean }) {
  return (
    <div
      className={cn(
        "rounded-xl border border-border bg-card p-4 hover:border-primary/50 transition-colors",
        layout === "stack" && "h-56",
